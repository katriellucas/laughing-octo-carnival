...
import { formatByteSize, toPosixPath } from "./utils.js";
 ...
 */
function syncLocalSimulation(binDir, storeName, publishId, collectionName, kvAssets, kvIndex) {
	...
	for (const asset of kvAssets) {
		/** @type {{ suffix: string, encoding: string | null }[]} */
		...
		for (const spec of variantSpecs) {
			...
			const key = `${publishId}_files_sha256:${blobName}`;
	...
	localMap[`${publishId}_index_${collectionName}`] = {
	...
	localMap[`${publishId}_metadata_${collectionName}`] = { data: "{}" };
 ...
 * @param {string} collectionName
 */
function emitVirtualModules(tempDir, inlinedLines, kvStoreName, publishId, collectionName) {
	writeFileSync(
	...
	writeFileSync(
		...
		`${[
			...
			`export const publishId = ${JSON.stringify(publishId)};`,
...
export async function processAssets(builder, options) {
	const { temp, publishDir, binDir, wasmAssetLimit, kvStoreName, publishId, collectionName } =
		options;

	if (!existsSync(publishDir)) {
		emitVirtualModules(temp, [], kvStoreName, publishId, collectionName);
	...
	syncLocalSimulation(binDir, kvStoreName, publishId, collectionName, kv, kvIndex);
	emitVirtualModules(temp, lines, kvStoreName, publishId, collectionName);
...
export function logAssets(inlined, kv, builder) {
	...
	const padding = [...inlined, ...kv].reduce((max, asset) => {
		return Math.max(max, asset.route.length);
	}, 0);
