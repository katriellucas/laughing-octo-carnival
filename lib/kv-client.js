...
export class FastlyKVClient {
	...
	async resolveStoreId() {
		...
		const store = data.find(
			(/** @type {{ name: string, id: string }} */ s) => s.name === this.storeName
		);
	 ...
	 */
	async syncAssets(assets, kvIndex, storeContentDir, publishId, collectionName, builder) {
		builder.log.minor(`Syncing assets to Fastly KV Store "${this.storeName}"...`);

		for (const asset of assets) {
			const variants = [
				{ key: `${publishId}_files_sha256:${asset.hash}`, suffix: "", encoding: null },
				...asset.variants.map((encoding) => ({
					key: `${publishId}_files_sha256:${asset.hash}_${encoding}`,
					suffix: `_${encoding}`,
					encoding,
				})),
			];
		...
		const indexKey = `${publishId}_index_${collectionName}`;
		...
		const metadataKey = `${publishId}_metadata_${collectionName}`;
