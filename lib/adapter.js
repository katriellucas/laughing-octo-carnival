...
import { logAssets, processAssets } from "./assets.js";
...
async function syncRemoteKV(
	...
	try {
		await client.syncAssets(assets, kvIndex, contentDir, publishId, collectionName, builder);
	} catch (err) {
	...
	writeFileSync(
		...
		`${JSON.stringify({ storeName, publishId, collectionName }, null, 2)}\n`
	);
}
